...
public class EnergyRibbonMultiCircleController : MonoBehaviour
{
    [Header("噪聲")]
    ...
    [SerializeField, Range(0f, 0.5f)]  float _ringRadius = 0.35f;
    ...
    float   _lastBranchSpread, _lastAmplitude, _lastThickness, _lastRingRadius, _lastAlpha;
    bool    _initialized;
    ...
    static readonly int PRingRadius   = Shader.PropertyToID("_RingRadius");
    ...
    void OnEnable()
    {
        ...
        if (_graphic == null)
        {
            Debug.LogWarning($"{nameof(EnergyRibbonMultiCircleController)} 需要掛在有 Image / RawImage 的物件上。", this);
            return;
        }
        ...
        if (_mat == null)
        {
            _mat = _graphic.material != null
                 ...
                 : new Material(Shader.Find("UI/EnergyRibbonCircle"));
    ...
    void ApplyAll()
    {
        ...
        if (!_initialized || _ringRadius != _lastRingRadius) { _mat.SetFloat(PRingRadius, _ringRadius); _lastRingRadius = _ringRadius; }
